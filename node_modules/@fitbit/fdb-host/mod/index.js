...
import { InvalidParams, ParseJSON, Peer, StringifyJSON, TypesafeRequestDispatcher, } from '@fitbit/jsonrpc-ts';
...
var Host = (function (_super) {
    ...
    function Host(hostInfo, timeout) {
        ...
        _this.dispatcher
            ...
            .method('initialize', FDBTypes.InitializeParams, _this.handleInitialize)
            .method('app.install.stream.begin', FDBTypes.AppInstallStreamBeginParams, _this.handleAppInstallBegin)
            .method('app.install.stream.finalize', FDBTypes.StreamCloseParams, _this.handleAppInstallFinalize)
            .method('app.install.stream.abort', FDBTypes.StreamCloseParams, _this.handleAppInstallAbort);
    ...
    Host.prototype.validateAppInstallStream = function (stream) {
        ...
        if (stream !== this.appInstallStream.token) {
            throw new InvalidParams('Stream token does not match the current app install stream', { stream: stream });
        }
    };
    return Host;
}(EventEmitter));
