...
function install(stores) {
    return (cli) => {
        ...
        cli
            ...
            try {
                if (hasApp && appHost) {
                    await sideload.app(appHost.host, appPackage, makeProgressCallback('app'));
                    cli.ui.redraw.done();
                }
                if (hasCompanion && companionHost) {
                    await sideload.companion(companionHost.host, appPackage, makeProgressCallback('companion'));
                    cli.ui.redraw.done();
                }
            }
            catch (ex) {
