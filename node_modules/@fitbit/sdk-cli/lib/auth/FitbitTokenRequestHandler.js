...
class FitbitTokenRequestHandler {
    ...
    performRevokeTokenRequest(configuration, request) {
        return fetch(configuration.revocationEndpoint, Object.assign({}, commonParams, { body: this.utils.stringify({
                token: request.token,
            }) })).then(response => response.ok);
    }
    async performTokenRequest(configuration, request) {
        const response = await fetch(configuration.tokenEndpoint, Object.assign({}, commonParams, { body: this.utils.stringify(request.toStringMap()) }));
