...
function validateIcon({ projectConfig, onDiagnostic, }) {
    ...
    const validateStream = new stream_1.default.Transform({
        objectMode: true,
        transform(file, _, cb) {
            if (file.isNull() || file.relative !== path_1.default.normalize(projectConfig.iconFile)) {
            ...
            if (file.isBuffer()) {
                getPNGDimensions(file.contents)
                    ...
                    if (iconWidth !== EXPECTED_ICON_WIDTH || iconHeight !== EXPECTED_ICON_HEIGHT) {
                        ...
                        return cb(new plugin_error_1.default(PLUGIN_NAME, errorMessage, { fileName: file.relative }));
                    }
                    cb(undefined, file);
                })
                ...
                cb(new plugin_error_1.default(PLUGIN_NAME, 'Icon file is not a buffer', { fileName: file.relative }));
            }
        },
        flush(cb) {
            if (!iconExists) {
