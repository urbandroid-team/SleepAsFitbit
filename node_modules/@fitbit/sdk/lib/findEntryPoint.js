...
function findEntryPoint(possibilities, options) {
    ...
    if (foundEntryPoints.length === 1)
        ...
        if (notFoundIsFatal) {
            throw new Error(`No ${component} entry point found! None of ${possibilitiesStr} are present in the project.`);
        }
        else {
            onDiagnostic({
        ...
        throw new Error(`Multiple ${component} entry points found! ${humanize_list_1.default(foundEntryPoints)} are all valid entry points; rename all but one and try again.`);
    }
    return entryPoint;
}
